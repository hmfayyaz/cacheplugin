...
final class FLCustomizer {
	...
	static public function _get_less_vars() {
		...
		$vars['form-input-bg-color'] = FLColor::hex( $mods['fl-form-input-bg-color'] );
		...
		if ( 'transparent' === $vars['form-input-bg-color'] ) {
			$vars['input-bg-color'] = FLColor::hex( array( $vars['content-bg-color-2'], $vars['body-bg-color-2'], '#fcfcfc' ) );
		} else {
			$vars['input-bg-color'] = FLColor::hex_or_transparent( $vars['form-input-bg-color'] );
		}
		...
		// check for FA plugin.
		if ( class_exists( 'FLBuilderFontAwesome' ) ) {
			if ( FLBuilderFontAwesome::is_installed() ) {
				$vars['font-awesome-family']  = sprintf( "'Font Awesome %s", floor( (float) fa()->version() ) );
				$vars['font-awesome-family'] .= FLBuilderFontAwesome::is_pro_enabled( false ) ? " Pro'" : " Free'";
			}
		}
