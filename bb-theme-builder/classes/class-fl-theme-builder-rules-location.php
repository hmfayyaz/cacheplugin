...
if ( ! class_exists( 'FLPageWalker' ) ) {
	...
	class FLPageWalker extends Walker_Page {
		...
		function start_el( &$output, $page, $depth = 0, $args = array(), $current_page = 0 ) {
			$mdash   = str_repeat( '&mdash; ', $depth );
			$title   = wp_slash( esc_attr( preg_replace( '/[^\x20-\x7E]*/', '', $page->post_title ) ) );
			$title   = empty( $title ) ? __( 'No Title', 'bb-theme-builder' ) : $title;
			$output .= '{'
				. '"id": ' . $page->ID . ','
				. '"name": "' . $title . '",'
				. '"label": "' . $mdash . $title . '",'
...
final class FLThemeBuilderRulesLocation {
	...
	static public function get_all_saved( $post_type = null ) {
		...
		/**
		 * Returns an array of all of the saved locations for a post type
		 * @see fl_theme_builder_saved_locations_for_type
		 */
		return apply_filters( 'fl_theme_builder_saved_locations_for_type', $locations, $post_type );
	}
	...
	static public function get_saved( $post_id, $sorted = false ) {
		...
		/**
		 * Returns the location data for a post id
		 * @see fl_theme_builder_saved_locations_for_post_id
		 */
		return apply_filters( 'fl_theme_builder_saved_locations_for_post_id', $saved, $post_id );
	}
	...
	static public function get_saved_exclusions( $post_id, $sorted = false ) {
		...
		/**
		 * Returns the location exclusion data for a post.
		 * @see fl_theme_builder_saved_exclusions_for_post_id
		 */
		return apply_filters( 'fl_theme_builder_saved_exclusions_for_post_id', $saved, $post_id );
	}
		...
		foreach ( $post_types as $post_type_slug => $post_type ) {
			...
			if ( 'post' == $post_type_slug || ! empty( $post_type_object->has_archive ) ) {
				...
				$by_template_type['archive'][ $post_type_slug ] = array(
					'id'    => $post_type_slug,
					/* translators: %s: singular post type name */
					'label' => sprintf( __( '%s Archives', 'bb-theme-builder' ), esc_html( $post_type->labels->singular_name ) ),
					'type'  => 'archive',
				);
