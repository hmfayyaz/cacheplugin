		...
		if ( $bail_out ) {
			...
			if ( 'user' == $object['type'] ) {
				...
				if ( 'ul' === $list_tag || 'ol' === $list_tag ) {
					$list_item_tag = 'li';
				} elseif ( 'span' === $list_tag ) {
					$list_item_tag       = 'span';
					$list_item_separator = '' === trim( $settings->list_item_separator ) ? '' : '<span class="fl-theme-builder-post-item-separator">' . esc_html( $settings->list_item_separator ) . '</span>';
				}
				...
				foreach ( $values as $post ) {
					...
					$class_list       = "post-$post_id" . ( '' === $list_item_separator ? '' : ' fl-theme-builder-post-item-has-separator' );
					$content_items[] .= "<$list_item_tag class='{$class_list}'>$list_item_text</$list_item_tag>";
				}

				$content = "<$list_tag>" . join( $list_item_separator, $content_items ) . "</$list_tag>";
				...
				if ( 'ul' === $list_tag || 'ol' === $list_tag ) {
					...
					$list_item_tag       = 'span';
					$list_item_separator = '' === trim( $settings->list_item_separator ) ? '' : '<span class="fl-theme-builder-taxonomy-term-separator">' . esc_html( $settings->list_item_separator ) . '</span>';
				}
				...
				if ( 'checkbox' == $object['field_type'] || 'multi_select' == $object['field_type'] ) {
					foreach ( $object['value'] as $term ) {
						...
						if ( $term_data ) {
							...
							if ( ! $skip_term ) {
								$content_items[] = "<{$list_item_tag} class='taxonomy-{$term_data->term_id}'>{$term_name}</{$list_item_tag}>";
							}
						}
					}
				} else {
				...
				$content .= join( $list_item_separator, $content_items );
				$content .= '</' . $list_tag . '>';
			}
		}
